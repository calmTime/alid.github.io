...
我们假设背包的最大承载重量是 5。我们有 3 个不同的物品，每个物品的重量分别是 `w = {2，4，3}` 

> 这里将不和大多数文章一样和递归进行比较, 我们直接使用动态规划的思想考虑问题, **也就是记录状态, 进而推出最终状态**

但是这里我们应记录怎样的状态呢? 这就是动态规划解法第一步需要得到的答案. 刚开始解除动态规划问题, 可能比较难直接理解状态, 这里我们从**拆解问题**说起. 回顾开篇我们说过的, 用小问题推出大问题. 这里怎么拆分小问题呢? 我们有2个条件, **物品**和**背包**. 拿什么物品是我们要做出的`选择`而背包体积我们可以进行`拆分`

1. 我们把整个求解过程`拆分`为 `w` 个阶段(w 表示背包的容积)，每次我们都会增大1点背包的容积, 并计算当前容积下背包最大可以放多少东西

2. 每个物品做出`选择`（放入或者不放入背包）之后，背包中的物品的重量会有多种情况，也就是说，会达到多种不同的**状态**. 而每一层的状态个数也就是我们可以尝试是否放进背包的物品个数 `n` 个（n 表示可选择物品的个数）

这里拆分任务, 并作出选择后, 我们就可以得到每一步的状态. 我们可以用一个二维数组 `dp[w+1][n+1]`，来记录每次可以计算出的状态。这就是第一个核心点 `记录状态`. 而`dp[i][j]`记录的状态就是在**容积为 i** 并且**使用 j** 件物品的时候能达到的最大容积.

我们可以**基于上一层的状态**集合，来**推导下一层的状态**集合. 则也就是建立`转移方程`的过程.

把记录状态的数组如果画成一个表就可以更清晰的表述由之前的状态推出后面的状态的过程.

![img](/img/in-post/post-dynamic-planning/dp-table.png)

我们来分析一下上图, 每次我们在考虑是否将当前物品放进去的时候, 看到会比较放入当前总重量比较大还是不放入当前总重量比较大. 我们通过分析两个case说明怎样进行状态转移
